...
import { Expand, FileType, Replace } from 'lucide-react'
...
function LinkMore({ pathname }: LinkMoreProps) {
    return (
        <>
            <Popover>
                <PopoverTrigger className='focus:bg-accent focus:text-accent-foreground data-[variant=destructive]:text-destructive data-[variant=destructive]:focus:bg-destructive/10 dark:data-[variant=destructive]:focus:bg-destructive/20 data-[variant=destructive]:focus:text-destructive data-[variant=destructive]:*:[svg]:!text-destructive relative flex cursor-default items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-hidden select-none' >
                Mais Ferramentas
                </PopoverTrigger>
